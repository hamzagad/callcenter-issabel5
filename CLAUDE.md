...
Issabel Call Center module (v4.0.0.5) for Issabel 5 - a predictive dialer and call center solution built on Asterisk PBX. Runs on Rocky 8 with PHP 7.x, Asterisk 16/18, and MySQL/MariaDB.
...
| Issue | Location | Fix |
|-------|----------|-----|
| `__autoload()` | `setup/dialer_process/dialer/dialerd:33` | Replace with `spl_autoload_register()` |
| `ereg()` | `modules/client/libs/paloSantoUploadFile.class.php:114` | Use `preg_match()` |
| Curly brace `$str{0}` | `modules/agent_console/libs/JSON.php` (~60 places) | Use `$str[0]` |
| Curly brace `$str{0}` | `modules/client/libs/paloSantoUploadFile.class.php:112` | Use `$str[0]` |
| `Services_JSON` library | `modules/agent_console/libs/JSON.php` | Replace with native `json_encode()`/`json_decode()` |

The `VALIDATION_TYPE => "ereg"` config values (42 occurrences) depend on the Issabel framework's validation handling.
...
- Logs: `/opt/issabel/dialer/dialerd.log`
...
- Agent types: `Agent` (legacy), `SIP`, `IAX2`, `PJSIP`

## Community

Maintained by Issabel community. Support: https://t.me/IssabelPBXip
