var module_name = 'rep_outgoing_campaigns_panel';
...
var STORAGE_KEY_SHIFT_FROM = 'outgoing_panel_shift_from';
var STORAGE_KEY_SHIFT_TO = 'outgoing_panel_shift_to';
...
function updateShiftRangeIndicator() {
    var indicator = $('#outgoingShiftRangeIndicator');
...
function applyShiftFilter() {
    var rawFrom = $('#outgoingShiftFromHour').val();
    var rawTo = $('#outgoingShiftToHour').val();
...
function updateStats(statuscount, stats) {
    ...
    if (statuscount) {
        $('#outgoing_stat_total').text(statuscount.total || 0);
        $('#outgoing_stat_onqueue').text(statuscount.onqueue || 0);
        $('#outgoing_stat_success').text(statuscount.success || 0);
        $('#outgoing_stat_abandoned').text(statuscount.abandoned || 0);
        $('#outgoing_stat_finished').text(statuscount.finished || 0);
        $('#outgoing_stat_losttrack').text(statuscount.losttrack || 0);
    }

    // Update duration stats
    if (stats) {
        $('#outgoing_stat_maxduration').text(formatTime(stats.max_duration || 0));
        ...
        $('#outgoing_stat_avgduration').text(formatTime(avgSec));
    }
}

function updateActiveCalls(calls) {
    var tbody = $('#outgoingActiveCallsBody');
    tbody.empty();
...
$(document).ready(function() {
    ...
    $('#outgoingShiftFromHour').val((shiftFromHour < 10 ? '0' : '') + shiftFromHour);
    $('#outgoingShiftToHour').val((shiftToHour < 10 ? '0' : '') + shiftToHour);

    // Update the shift range indicator
    updateShiftRangeIndicator();

    // Bind apply button click handler
    $('#applyOutgoingShiftFilter').on('click', function() {
        applyShiftFilter();
    });
